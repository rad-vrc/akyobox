...
- `highScore`: number (ローカル保存されたハイスコア)
  - `localStorage` の `gameHighScore` キーに保存する
  - App 起動時に `useEffect` で `localStorage.getItem('gameHighScore')` を読み込み、存在すれば数値にパースして state を初期化
  - ゲーム終了時（GameOverScreen 表示時）に現在のスコアと `highScore` を比較し、新しいスコアが大きい場合は `localStorage` と state の両方を更新
...
**UI仕様:**
- ヒーローセクションでタイトルロゴと 1 行説明「30秒間にデビルヤギAkyoを叩いて得点を稼ごう！」を表示
- 「How to Play」カードを配置し、ターゲット/デコイ画像のサムネイル、制限時間、操作方法（クリック/タップ）を図解
- ミニチュートリアルは最小2ステップに統一し、以下の PNG または WebP 画像（GIF は非推奨）でターゲットを叩く流れを可視化
  1. 「正解画像をクリック」（`step1_target.png`）
  2. 「デコイ画像を避ける」（`step2_decoy.png`）
- 各ステップ画像の推奨サイズは 600×400px、ファイルサイズは 200KB 以下とし、モバイル環境でも読み込みが重くならないようにする
- 各ステップ画像には操作説明を含む `alt` テキスト（例: 「正解画像をクリックする様子」「デコイ画像を避ける様子」など）を付与する
...
- SettingsScreen はフルスクリーンオーバーレイのモーダルとして表示し、背景スクロールをロックする
- モーダル表示時は `useEffect` で最初のフォーカス可能要素（例: 閉じるボタン）にフォーカスを移動し、`Esc` キーで閉じられるようにする
- `Tab` / `Shift+Tab` キー入力を監視し、フォーカスがモーダル外へ抜けないようにループさせる（フォーカストラップ）
- モーダルを閉じる際は、モーダルを開いたトリガーボタン（StartScreen の「設定」ボタンなど）にフォーカスを戻す
- 実装には `focus-trap` や `react-focus-lock` などの既存ライブラリの利用を推奨し、独自実装を行う場合も同等のフォーカス管理とアクセシビリティ要件を満たす
...
⚠️ **注意:** ライト/ダークなど異なるテーマ間で色を混在させたり、`#ffffff` と `#f0f0f0`、`#000000` と `#1a1a1a` のようなコントラスト比が 3:1 未満の近似色同士を組み合わせないこと。各テーマ内で上記のような十分なコントラスト比を満たすペアのみを使用する。

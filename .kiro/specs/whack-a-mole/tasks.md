...
- [ ] 6. AkyoItemコンポーネントの実装
  ...
  - `React.memo` と `useCallback` を適用し、props 変化時のみ再レンダリング
  - `prefers-reduced-motion` に応じてアニメーションを無効化する CSS を追加
...
- [ ] 8. 得点計算とタイマーロジックの実装
  ...
  - 得点イベントに連動したサウンド／ハプティック API（対応ブラウザのみ）の発火処理とミュート設定の保持
...
- [ ] 11. カスタム画像管理機能の実装
  ...
  - Canvas API を用いた WebP 変換 + Base64エンコーディング
...
- [ ] 14. パフォーマンス最適化と画像読み込み戦略
  - **コンポーネント最適化:**
    ...
    - requestAnimationFrame バッチング・Passive Event Listener を導入して INP を抑制
  - **リソース管理:**

...
- `fallback.png` - エラー時のフォールバック画像（オプション）
...
- [ ] 1. プロジェクト基盤とデータモデルの作成
  ...
  - **チェックポイント:** 型定義がエラーなくコンパイルでき、ユーティリティ関数が期待通りの値を返すことを確認
...
- [ ] 2. 基本的なゲーム状態管理の実装
  ...
  - **チェックポイント:** 状態遷移が正しく動作し、最低限のゲームループ（開始→プレイ中→終了）が成立していることを確認

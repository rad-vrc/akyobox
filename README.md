...
### データ管理 (Vercel KV CLI)
テストデータの削除やリセットは、Vercel管理画面の **Storage > CLI** からRedisコマンドで行います。

**よく使うコマンド:**
*   `KEYS *`: 保存されている全キーを表示します。
*   `ZRANGE highscores 0 -1 WITHSCORES`: ランキングの全データをスコア付きで表示します。
*   `GET detail:user:UUID`: 特定ユーザーの詳細データ（名前など）を確認します。
*   `DEL detail:user:UUID`: 特定ユーザーの詳細データを削除します。
*   `ZREM highscores user:UUID`: 特定ユーザーをランキングから削除します。
*   `FLUSHDB`: **全データを削除**します（初期化）。
...
### 2. UI/UX 最適化
*   **フォント最適化**: Next.jsの `next/font/local` を導入し、カスタムフォント「Oshigo」をレイアウトシフトなしで高速に表示しています。
*   **レスポンシブ表示 (Desktop)**:
    *   ブラウザのウィンドウサイズに合わせて、アスペクト比(16:9)を維持したまま自動的に拡大縮小します。1920x1080以下の画面でも見切れることなくプレイ可能です。
*   **キーボード干渉対策**: Unityによるキーボード入力の独占を防ぎ、名前入力時のバックスペースや、`Ctrl` + スクロールによる画面ズーム操作が正常に機能するように調整済みです。

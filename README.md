...
-   イントロBGM（約10秒）を流し、テキスト/TMPとノイズオーバーレイを表示。イントロ中はタイマー停止・Akyoスポーン停止
-   サイレン SE（30 秒/15 秒の 2 段階）
-   テレグラフ演出（出現前に膨らみ）
-   コンボボーナス（1 秒以内連続ヒットで倍率加算）
-   当たり出現率: `targetProbability ≈ 0.66`（当たり2:ハズレ1）。ハズレ率は維持したまま当たりを増量
-   1ウェーブ当たり出現数倍率: `waveCountMultiplier = 2`（全体発生数を倍増）
-   ゲーム終了時にコンボリセット（再開で前プレイのコンボが残らない）
